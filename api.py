...
class CaptionDirectoryRequest(BaseModel):
    ...
    prompt: str
    append_prompt: Optional[str] = None
    previewDoNotUpdate: Optional[bool] = False
...
@app.get("/available_models", tags=["info"])
...
@app.get("/available_prompts", tags=["info"])
async def get_available_prompts():
    return {"available_prompts": service_selection.available_prompts}

@app.get("/caption_directory_status", tags=["info"])
...
@app.post("/load_model_service", tags=["actions"])
...
@app.post("/stop_model_service", tags=["actions"])
...
@app.post("/caption_directory", tags=["actions"])
        ...
        def process_files():
            for file in files:
                ...
                try:
                    ...
                    if not request.previewDoNotUpdate:
...
# individual image captioning, does not create a caption file like the caption directory function does
@app.post("/caption", tags=["actions"])
...
@app.post("/stop_job", tags=["actions"])
async def stop_job():
    global caption_job
